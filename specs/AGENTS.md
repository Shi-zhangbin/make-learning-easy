...
```
T0 ──→ T1 ──→ T3 ──→ TTS实测 ──→ T2 ──→ T4+T5 ──→ T6 ──→ T7
选题   知识点   口播稿              PPT     素材      HyperFrames 渲染+
研究   大纲     先行              大纲+    搜刮+     compositions 配音
                                    分镜    配图
```
...
| **T4** | **启明(Python脚本)** | — | terminal(background) | **无自主权**，纯下载 |
| **T5** | chuangzuo | MiniMax-M2.7 | terminal(background) | **无自主权**，按prompt生成 |
...
- ✅ 必须：ffprobe测量实际时长
- ✅ 必须：用 `atempo=1.15,volume=6dB` 加速
...
- [ ] 加载本AGENTS.md，确认最新版本
- [ ] 与主人确认审核节奏：全自动 / 分阶段审核
- [ ] 确认TTS语速参数（当前：atempo=1.15x, volume=6dB）
...
| 参数 | 值 | 说明 |
|------|----|------|
| 语速 | atempo=1.15x + volume=6dB | 有效语速5.26字/秒 |
| 目标字数 | 时长×5.26 | ±5%容差 |
| 帧率 | 15fps | 30fps超时 |
| 画面 | 1920×1080 | |
